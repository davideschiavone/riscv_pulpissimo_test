...
#define util_check_rw_fc(csr, addr, i, act, last) \
  for (i = 0; i < (sizeof(g_ ## csr ## _a)/4); i++) { \
    util_csrrw(addr, g_ ## csr ## _a[i], last); \
    util_csrr (addr, act); \
    check_uint32(result, "csrrw_" #csr, act, g_ ## csr ## _exp[i]); \
...
#define util_check_rw_mhartid_fc(csr, addr, i, act, last) \
  for (i = 0; i < (sizeof(g_ ## csr ## _a)/4); i++) { \
    util_csrrw(addr, g_ ## csr ## _a[i], last); \
    util_csrr (addr, act); \
    check_uint32(result, "csrrw_" #csr, act, 0x3E0); \
    if (i != 0) \
      check_uint32(result, "csrrw_prev_" #csr, last, 0x3E0); \
  }

#define util_check_rw_mhartid(csr, addr, i, act, last) \
  for (i = 0; i < (sizeof(g_ ## csr ## _a)/4); i++) { \
    util_csrrw(addr, g_ ## csr ## _a[i], last); \
    util_csrr (addr, act); \
    check_uint32(result, "csrrw_" #csr, act, 0x00); \
    if (i != 0) \
      check_uint32(result, "csrrw_prev_" #csr, last, 0x00); \
  }
...
void check_csr_rw(testresult_t *result, void (*start)(), void (*stop)()) {
  ...
  if (plp_isFc()) {
    util_check_rw_mhartid_fc(mhartid, "0xF14", i, act, last);
  } else {
    util_check_rw_mhartid(mhartid, "0xF14", i, act, last);
  }
}
